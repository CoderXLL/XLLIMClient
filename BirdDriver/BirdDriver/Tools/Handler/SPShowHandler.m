...
@interface SPShowHandler()

@end

@implementation SPShowHandler

static NSNumberFormatter *shareNumFormatter;

#pragma mark - 展示数据

+ (NSString *)fen2Yuan:(NSInteger)fen{
    NSString *resStr = @"0.00";
    if (fen && fen!=0) {
        NSNumberFormatter *numFormatter = [SPShowHandler shareNumFormatter];
        numFormatter.numberStyle = kCFNumberFormatterDecimalStyle;
        numFormatter.positiveFormat = @"###,##0.00;";
        resStr = [numFormatter stringFromNumber:@(fen/100.00)];
    }
    return resStr;
}

+ (NSNumberFormatter *)shareNumFormatter{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        shareNumFormatter = [[NSNumberFormatter alloc] init];
    });
    return shareNumFormatter;
}

+ (NSString *)formatCardNumber:(NSString *)cardNum{
    NSCharacterSet *characterSet = [NSCharacterSet characterSetWithCharactersInString:@"0123456789\b"];
    NSString *string = [cardNum stringByReplacingOccurrencesOfString:@" " withString:@""];
...
+ (NSString *)encryptCardNumber:(NSString *)cardNum{
    ...
    NSArray *firstNum = [cardNum componentsSeparatedByCharactersInSet:invertedSet];
    NSString *numStr = [firstNum componentsJoinedByString:@""];
    
    //2.取头尾长度
    NSInteger headLength = 4;
    NSInteger tailLength = numStr.length%4?numStr.length%4:4;
    
    //3.中间部分换*
    NSString *continuousStr = [NSString stringWithString:numStr];
    NSInteger starLength = numStr.length - headLength -tailLength;
    if (starLength>0) {
        for (int i = 0; i < starLength; i++) {
            continuousStr = [continuousStr stringByReplacingCharactersInRange:NSMakeRange(headLength+i, 1) withString:@"*"];
        }
        
    //4.加空格
    NSString *finalStr = [SPShowHandler formatCardNumber:continuousStr];
    return finalStr;
...
+ (NSString *)dotDeviceName{
    ...
    NSString *result = [machineString stringByReplacingOccurrencesOfString:@","
                                                                withString:@"."];
    return result;
}
...
/**
 ...
 @return 苹果设备内部描述转码成常见描述
 */
+ (NSString *)currentDeviceName {
    DiviceType type = [self transformMachineToIdevice];
    NSString *device = iDeviceNameContainer[type];
    return device;
}
...
static NSString *iDeviceNameContainer[] = {
    ...
    [iPhone_4]                  = @"iPhone 4",
    [iPhone_4_Verizon]          = @"iPhone 4 (Verizon)",
    [iPhone_4S]                 = @"iPhone 4S",
