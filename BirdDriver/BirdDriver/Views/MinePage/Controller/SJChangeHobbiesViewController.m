//
//  SJChangeHobbiesViewController.m
//  BirdDriver
//
...
@interface SJChangeHobbiesViewController ()
...
@implementation SJChangeHobbiesViewController
    ...
    [JAMessagePresenter postDataDictionary:@"hobbyTags"
                                    Result:^(NSMutableArray * _Nullable array) {
                                        dispatch_async(dispatch_get_main_queue(), ^{
                                            ...
                                            self.hobbyTextFieldView.frame = CGRectMake(0, self.selectView.frame.origin.y + self.selectView.frame.size.height + 10, kScreenWidth, 150);
                                            self.mainScrollView.contentSize = CGSizeMake(kScreenWidth, self.hobbyTextFieldView.frame.origin.y);
                                        });
    }];
}
...
//    mainScrollView.contentSize = CGSizeMake(kScreenWidth, self.hobbyTextFieldView.frame.origin.y + self.hobbyTextFieldView.frame.size.height);
    mainScrollView.contentSize = CGSizeMake(kScreenWidth, self.hobbyTextFieldView.frame.origin.y);
...
- (UIView *)headerView{
    if (!_headerView) {
        ...
        titleLabel.textColor = [UIColor colorWithHexString:@"2B3248" alpha:1];
...
-(UIView *)selectView{
    if (!_selectView) {
        ...
        selectTitleLabel.font=[UIFont boldSystemFontOfSize:15];
        [_selectView addSubview:selectTitleLabel];
        
        SJLineView *sepLine = [[SJLineView alloc] initWithFrame:CGRectMake(15, CGRectGetMaxY(selectTitleLabel.frame)+20, kScreenWidth-30, 1)];
        sepLine.backgroundColor = [UIColor colorWithHexString:@"EEEEEE" alpha:1];
        [_selectView addSubview:sepLine];
        
        
        GBTagListView *selectItems=[[GBTagListView alloc]initWithFrame:CGRectMake(0,30 + 15 + 20+10, kScreenWidth - 15, 0)];
